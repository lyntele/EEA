#!/usr/bin/env python3
"""Replay post-selection runtime trigger through rewrite for saved cases.

This probe reads saved ``eea_runtime_request.json`` files from DeepEye
post-selection runs. Runtime planning remains answer-blind; gold SQL is read
only after rewrite for evaluation.
"""
...
def _gold_sql_from_case_dir(case_dir: Path) -> str:
    for name in ("eea_update_request.json", "eea_official_reconcile_request.json"):
        ...
        try:
            ...
            continue
        for key in ("gold_sql", "gold", "reference_sql"):
            sql = str(payload.get(key) or "").strip()
            if sql:
                return sql
    return ""
...
def main() -> int:
    ...
    parser.add_argument("--row_sample_limit", type=int, default=500)
    ...
    for case_dir in sorted(work_root.glob("qid_*"), key=_case_sort_key):
        ...
        try:
            ...
            gold_sql = _gold_sql_from_case_dir(case_dir)
            ...
            pred_cmp = (
                ...
                if gold_sql and selected_sql
                else None
            )
            rewrite_result: Dict[str, Any] = {}
            rewrite_cmp = None
            compiler_output = plan.get("compiler_output")
            if reason == "ready" and compiler_output is not None:
                rewrite_result = rewrite_one_candidate(
                    ...
                    natural_language_hint=str(plan.get("repair_brief") or "")
                    or str(plan.get("instantiated_hint") or ""),
                )
                rewrite_sql = str(rewrite_result.get("rewrite_sql") or "").strip()
                if rewrite_sql and gold_sql:
                    rewrite_cmp = run_execution_comparison(
                        db_path=str(db_path),
            ...
            row.update(
                {
                    ...
                    "rewrite_result": _payload(rewrite_result),
                    "rewrite_realization_origin": (
                        rewrite_realization_origin_from_result(rewrite_result)
                        if rewrite_result
                        else ""
                    ),
    ...
    summary = {
        ...
        "rewritten_cases": [
            row.get("question_id") for row in rows if row.get("rewrite_sql")
        ],
        "rewrite_equivalent_cases": [
            row.get("question_id") for row in rows if row.get("rewrite_status") == "equivalent"
        ],
        "regression_cases": [
            row.get("question_id")
            for row in rows
            if row.get("pred_status") == "equivalent"
            and row.get("rewrite_status") not in {"equivalent", "not_run"}
        ],
        "wuv2_2_disabled_reason_count": sum(
