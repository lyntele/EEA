...
OUT_BASE=/data/liuyining/ace4sql/method/EEA/rulebook/outputs/retrieval_root_evidence_v11
GROUND_TRUTH=/data/liuyining/ace4sql/method/EEA/rulebook/scripts/probes/manual_pattern_ground_truth.json
LOG_DIR=$OUT_BASE/logs
mkdir -p "$LOG_DIR"

# Largest databases first so xargs -P 6 keeps the workers balanced.
DBS=(
  card_games
  codebase_community
  formula_1
...
run_one() {
  ...
  PYTHONPATH=/data/liuyining/ace4sql python3 method/EEA/rulebook/cli/run_online_e2e_validation.py \
    ...
    --skip_rewrite --skip-final-freeze \
