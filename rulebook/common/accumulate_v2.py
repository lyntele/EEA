"""v2 Accumulate chain —— ErrorInstance → GroupSummary singleton formation.
...
函数本身在这里独立实现，便于在 pipeline_v2 的开发期 ErrorInstance→Compiler 链路
...
from .repair_program_normalizer_v2 import attach_canonical_repair_ir
...
from .trigger_contract_v2 import ensure_materialized_trigger_contract
...
_V2_ACCUMULATE_FAILURE_STREAK: List[dict[str, str]] = []


def _v2_accumulate_fail_fast_threshold() -> int:
    """连续失败阈值；0 表示禁用 fail-fast。"""
    raw = str(os.getenv("EEA_V2_ACCUMULATE_FAIL_FAST_AFTER", "3") or "").strip()
    if not raw:
        return 3
...
def _reset_v2_accumulate_failure_streak(*, db_id: str) -> None:
    if _V2_ACCUMULATE_FAILURE_STREAK and _V2_ACCUMULATE_FAILURE_STREAK[0]["db_id"] == db_id:
        _V2_ACCUMULATE_FAILURE_STREAK.clear()


def _record_v2_accumulate_failure(*, db_id: str, case_id: str, reason: str) -> Optional[str]:
    """记录连续失败；达到阈值时返回 fail-fast 消息。"""
    threshold = _v2_accumulate_fail_fast_threshold()
    if threshold <= 0:
        return None
    if _V2_ACCUMULATE_FAILURE_STREAK and _V2_ACCUMULATE_FAILURE_STREAK[0]["db_id"] != db_id:
        _V2_ACCUMULATE_FAILURE_STREAK.clear()
    _V2_ACCUMULATE_FAILURE_STREAK.append(
        {
            "db_id": db_id,
            "case_id": str(case_id),
            "reason": reason,
    ...
    if len(_V2_ACCUMULATE_FAILURE_STREAK) < threshold:
        return None
    failures = "; ".join(
        f"{item['case_id']} ({item['reason']})" for item in _V2_ACCUMULATE_FAILURE_STREAK
    )
    return (
        "Aborting run after "
        f"{len(_V2_ACCUMULATE_FAILURE_STREAK)} consecutive v2 accumulate failures "
        f"(threshold={threshold}) for db_id={db_id}. "
        f"Failure streak: {failures}. "
        "Set EEA_V2_ACCUMULATE_FAIL_FAST_AFTER=0 to disable this guard."
    )
...
def accumulate_wrong_case(
    *,
    ...
    """
    # 延迟 import 避开循环依赖（pipeline_v2 import accumulate_v2，反之也会）
    from .pipeline_v2 import run_error_instance_pipeline
    ...
    try:
        ...
        fail_fast_message = _record_v2_accumulate_failure(
            db_id=db_id,
            case_id=str(case_id),
    ...
    _reset_v2_accumulate_failure_streak(db_id=db_id)
