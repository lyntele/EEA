...
- DeltaFlags 26 维（复用 common/structure_delta.compute_delta_flags）
...
- LocalSchemaView 构造（复用 local_schema.build_local_schema_view）
...
from method.EEA.rulebook.common.io.local_schema import DBSchemaAccess, build_local_schema_view
...
def compute_structure_bundle(pred_sql: str, gold_sql: str) -> Dict[str, Any]:
    ...
    from method.EEA.rulebook.common.analysis.structure_family import cached_ast_signature, repair_signature_dict_from_sql
    from method.EEA.rulebook.common.analysis.structure_delta import build_structural_delta
...
def preprocess_case(
    *,
    ...
    若 ``execution_comparison`` 已在 caller 端算好（通常由 case_pipeline 在调用前做完），
